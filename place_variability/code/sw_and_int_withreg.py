'''
similart to sw_and_int_one_sess, except added selected_inh change, by first doing regression on everyone
the problem is now regression no longer exclude trials
'''
import os
import sys
import traceback
...
# SAVE_FN='fr_map.p'
# name_l = ['sw_and_int']
name_l = ['sw_and_int_exclude_sw_largetick']
...
if PEN is not None:
    name_l.append(f'pen_{PEN:1.0e}')
if PVAL_THRESH is not None:
    name_l.append(f'pthresh_{PVAL_THRESH:1.0e}')
SAVE_FN = '_'.join(name_l) + '.p'
...
def analyze_data(data,*args,**kwargs):
    ...
    selected_inh_fr_trial_within_field_all = {}


    # do peer prediction first
    # mean_within_field_pred_all,glm_res_df_all,r2_all = pp.sweep_fit_glm_predict_rate_change(spk_beh_df,None,all_fields_one_sess,
    ...
    #                                 ti=ti)
    mean_within_field_pred_all,glm_res_df_all,r2_all =pp.sweep_fit_glm_predict_rate_change_exclude(spk_beh_df,all_sw_with_metrics,all_fields_one_sess,X_raw,
                                    index_within_to_trial_index_df,
                                    fr_map_trial_df_int_combined,
                                    cell_cols_d,                                        
                                    do_inh_only=True,
    ...
    for (tt,uid,field_id),row in all_fields_one_sess.loc[ti].iterrows():
        ...
        selected_inh_fr_trial_within_field_all = mean_within_field_pred_all
        selected_inh_fr_trial_within_field_ = selected_inh_fr_trial_within_field_all.loc[(tt,uid,field_id),:].dropna()
        selected_inh_fr_trial_within_field = selected_inh_fr_trial_within_field_.values
        selected_inh_fr_trial_within_field_diff = np.diff(selected_inh_fr_trial_within_field,append=np.nan)
        
        diff_l = {}
        diff_l['inh'] = inh_fr_trial_within_field_diff
        diff_l['selected_inh'] = selected_inh_fr_trial_within_field_diff
    ...
    #         pyr_diff = np.diff(pyr_fr_trial_within_field,n=kk,append=[np.nan]*kk)
        ...
        for kk in range(1,n_lasting_trial+1): # need to group columns of pyr and int to be together for downstream analysis
            int_diff = selected_inh_fr_trial_within_field[kk:] - selected_inh_fr_trial_within_field[:-kk]
            int_diff = np.append(int_diff,[np.nan]*kk)
            diff_l[f'selected_int_{kk}'] = int_diff
    ...
    all_sw_with_metrics_oneti_with_inh_change= pp.add_inh_fr_change_to_all_sw(all_sw_with_metrics,inh_fr_trial_within_field_all,per_field_metrics_one,
                                                                            selected_inh_fr_trial_within_field=mean_within_field_pred_all,ti=0)
