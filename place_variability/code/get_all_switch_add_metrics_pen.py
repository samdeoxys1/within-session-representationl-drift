...
SAVE_FN='all_sw_info_pen.p'#f'all_sw_info_{PF_FR_KEY}.p'#
...
def load_preprocess_data(session_path):
    ...
    sw_res_pelt = sdspos.main(session_path,force_reload=False,dosave=False,load_only=True)

    data = {'spk_beh_df':spk_beh_df,'cell_cols_d':cell_cols_d,'pf_params_recombined':pf_params_recombined,
            'sw_res_pelt':sw_res_pelt            
            }
    return data
...
def analyze_data(data,*args,**kwargs):
    ...
    all_sw_d_pen = {}
    all_sw_with_metrics_d_pen={}
    for pen,sw_res in sw_res_pelt.items():
        ...
        all_sw_d_pen[pen] = all_sw_d
        all_sw_with_metrics_d_pen[pen] = all_sw_with_metrics_d
    
    res = {'all_sw_d_pen':all_sw_d_pen,'all_sw_with_metrics_d_pen':all_sw_with_metrics_d_pen}
    return res
