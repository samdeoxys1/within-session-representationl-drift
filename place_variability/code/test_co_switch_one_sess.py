...
SAVE_DIR=''
SAVE_FN=f'co_sw_res_afterheadscan_{PF_FR_KEY}.p'#'sw_res_afterheadscan.p'#f'sw_res_afterheadscan_{PF_FR_KEY}.p'
n_shuffle = 1000
...
def load_preprocess_data(session_path):
    ...
    changes_df_all_task = sw_res['changes_df']
    data = {'changes_df_all_task':changes_df_all_task}
    return data
...
def analyze_data(data,*args,**kwargs):
    ...
    changes_df_all_task = data['changes_df_all_task']
    ...
    res = {'co_sw_pval':pval_all_task,
            'n_co_sw':n_pair_all_n_co_d_all_task,
            'n_co_sw_shuffle':n_pair_all_n_co_d_l_all_task
            }
    return res
