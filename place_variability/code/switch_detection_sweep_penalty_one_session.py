...
SAVE_DIR=''
SAVE_FN=f'penalty_sw_res_afterheadscan_{PF_FR_KEY}.p'#f'sw_res_afterheadscan_{PF_FR_KEY}.p'
# n_shuffle = 1000
...
def load_preprocess_data(session_path):
    ...
    X = pf_res['params_recombined'].loc[PF_FR_KEY]
    data = {'X':X}
    return data
...
def analyze_data(data,*args,**kwargs):
    ...
    res = tcp.detect_switch_sweep_penalty(X)
    return res
